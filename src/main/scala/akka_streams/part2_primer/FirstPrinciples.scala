...
object FirstPrinciples extends App {
  ...
  /**
    * Exercise
    * create a stream that takes the names of persons, then you will keep the first 2 names with length > 5 characters
    */
  val namesSource = Source(List("yui", "shiho", "nanako", "risa", "neru", "manaka", "konoka", "miku"))
  val namesFlow = Flow[String].filter(name => name.length > 5).take(2)
  val namesSink = Sink.foreach(println)
  namesSource.via(namesFlow).to(namesSink).run()
  ...
  nameSource.via(longNameFlow).via(limitFlow).to(namesSink).run()
